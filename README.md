...
Direct API Test: Directly interacts with the SolidityScan API to perform scans, allowing users to input parameters and view raw API responses.

Agent Test: Utilizes an agent-based system (with a worker configuration) to guide users through the scanning process by collecting parameters interactively.
...
Threat Scan (v2 API): Performs specialized threat analysis, which may return an initialization response for asynchronous processing.
...
Game SDK API key (GAME_API_KEY) for Agent Test mode
...
Upon running, you'll see a menu with three options:

Direct API Test: Test the SolidityScan API directly by manually entering scan parameters.

Agent Test: Interact with the agent, which guides you through the scanning process step-by-step.
...
Initialization response (scan ID, status) or complete results
...
Notes
Threat Scan Limitations: The v2 API for threat scans may return an initialization response, indicating that results will be delivered asynchronously to configured endpoints. Check the SolidityScan documentation for details on retrieving these results.
...
Built with the SolidityScan API for smart contract scanning.

Utilizes the game_sdk for agent-based functionality.
